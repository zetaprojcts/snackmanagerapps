...
export default function AddPayment() {
  ...
  const handleSave = async () => {
    ...
    if (!amount || numericAmount <= adminFee) return Alert.alert('Error', `Nominal harus lebih besar dari biaya admin (Rp ${adminFee})`);
      ...
      <View style={styles.content}>
        ...
        {isLoadingDevices ? <ActivityIndicator size="small" color={COLORS.primary} style={{ alignSelf: 'flex-start' }} /> : (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.deviceScroll} contentContainerStyle={{ gap: 10 }}>
            {activeDevices.map(device => (
              <Chip key={device.id} label={`${device.device_name} (${device.ewallet})`} isActive={selectedDeviceId === device.id} onPress={() => setSelectedDeviceId(device.id)} />
            ))}
          </ScrollView>
        )}
        ...
        {selectedDeviceId ? (
          <View style={styles.receipt}>
            <View style={styles.row}><Text style={styles.receiptText}>Penarikan Kotor</Text><Text style={styles.receiptText}>Rp {numericAmount.toLocaleString('id-ID')}</Text></View>
            <View style={styles.row}><Text style={styles.receiptText}>Admin ({selectedWallet})</Text><Text style={[styles.receiptText, { color: COLORS.warning }]}>- Rp {adminFee.toLocaleString('id-ID')}</Text></View>
            <View style={styles.totalRow}><Text style={styles.totalText}>Nominal Cair</Text><Text style={styles.totalAmount}>Rp {netAmount.toLocaleString('id-ID')}</Text></View>
          </View>
        ) : null}

        <TouchableOpacity style={[styles.btnSave, loading && { opacity: 0.7 }]} onPress={handleSave} disabled={loading}>
          <Save color="#FFF" size={20} style={{ marginRight: 8 }} />
          <Text style={styles.btnText}>{loading ? 'Memproses...' : 'Simpan Penarikan'}</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.card, borderWidth: 1, borderColor: COLORS.border, borderRadius: 12, paddingHorizontal: 15, marginBottom: 20 },
  currency: { fontSize: 18, fontWeight: '700', marginRight: 10 },
  input: { flex: 1, paddingVertical: 15, fontSize: 20, fontWeight: '600' },
  receipt: { backgroundColor: COLORS.card, padding: 15, borderRadius: 12, borderWidth: 1, borderColor: COLORS.border, marginBottom: 20 },
  row: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
  receiptText: { color: COLORS.textMuted },
  totalRow: { borderTopWidth: 1, borderTopColor: COLORS.border, paddingTop: 10, marginTop: 5, flexDirection: 'row', justifyContent: 'space-between' },
