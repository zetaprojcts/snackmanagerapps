...
export default function DeviceDetail() {
  ...
  const chartData = datesArray.map((date, index) => {
    ...
    const activeColor = metricTab === "income" ? COLORS.primary : COLORS.warning;
  ...
  const activities = useMemo(() => {
    const list = metricTab === "income" 
      ? incomes.map((i: any) => ({ type: "income", amount: Number(i.amount), trx_date: i.trx_date }))
      : payments.map((p: any) => ({ type: "payment", amount: Number(p.gross_amount), trx_date: p.trx_date }));
    return list.sort((a, b) => new Date(b.trx_date).getTime() - new Date(a.trx_date).getTime()).slice(0, 10);
  ...
  if (!device) return <View style={styles.centerContainer}><EmptyState title="Device Tidak Ditemukan" subtitle="Data perangkat tidak tersedia" /></View>;
    ...
    <>
      <View style={styles.container}>
        <Animated.View entering={FadeInDown} style={styles.header}>
          ...
          <TouchableOpacity onPress={() => router.push({ pathname: "/edit-device", params: { id: device.id } })}><Edit size={20} color={COLORS.primary} /></TouchableOpacity>
        </Animated.View>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          <Animated.View entering={FadeInUp} style={styles.deviceCard}>
            <View style={styles.deviceTop}>
              ...
              <View style={styles.deviceInfo}>
                ...
                <View style={[styles.statusChip, { backgroundColor: device.is_active ? COLORS.success : COLORS.danger }]}><Text style={styles.statusChipText}>{device.is_active ? "Aktif" : "Nonaktif"}</Text></View>
              </View>
            </View>
            <View style={styles.divider} />
            <View style={styles.infoRow}><Mail size={16} color={COLORS.textMuted} /><Text style={styles.infoText}>{device.email || "-"}</Text></View>
            <View style={styles.infoRow}><Wallet size={16} color={COLORS.textMuted} /><Text style={styles.infoText}>{device.ewallet || "-"}</Text></View>
          </Animated.View>
          ...
          <Animated.View entering={FadeInUp.delay(150)} style={styles.summaryRow}>
            <TouchableOpacity activeOpacity={0.8} style={[styles.summaryCard, isIncomeActive ? styles.cardActive : styles.cardInactive]} onPress={() => { animateLayout(); setMetricTab("income"); setSelectedIndex(null); }}>
              <Text style={[styles.summaryLabel, isIncomeActive ? styles.textWhite : styles.textDarkMuted]}>Pendapatan</Text>
              <Text style={[styles.summaryValue, isIncomeActive ? styles.textWhite : styles.textDark]}>Rp {totalIncome.toLocaleString("id-ID")}</Text>
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.8} style={[styles.summaryCard, isPaymentActive ? styles.cardActive : styles.cardInactive]} onPress={() => { animateLayout(); setMetricTab("payment"); setSelectedIndex(null); }}>
              <Text style={[styles.summaryLabel, isPaymentActive ? styles.textWhite : styles.textDarkMuted]}>Penarikan</Text>
              <Text style={[styles.summaryValue, isPaymentActive ? styles.textWhite : styles.textDark]}>Rp {totalPayment.toLocaleString("id-ID")}</Text>
            </TouchableOpacity>
          </Animated.View>

          <Animated.View entering={FadeInUp.delay(200)} style={styles.chartSection}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chartFilterContainer}>
          ...
          <Animated.View entering={FadeInUp.delay(250)} style={styles.activitySection}>
            <View style={styles.activityHeader}><Text style={styles.sectionTitle}>Aktivitas Terbaru</Text></View>
            {activities.length === 0 ? <EmptyState title="Belum Ada Aktivitas" subtitle={`Tidak ada data ${isIncomeActive ? "pendapatan" : "penarikan"}`} /> : 
              activities.map((item, index) => (
                <Animated.View key={`${item.type}-${item.trx_date}-${index}`} entering={FadeInUp.duration(300).delay(index * 40)} style={styles.activityItem}>
                  <View><Text style={styles.activityType}>{item.type === "income" ? "Pendapatan" : "Penarikan"}</Text><Text style={styles.activityDate}>{new Date(item.trx_date).toLocaleDateString("id-ID")}</Text></View>
...
const styles = StyleSheet.create({
  ...
  chartContainer: { backgroundColor: "#FFFFFF", borderRadius: 24, paddingHorizontal: 20, paddingBottom: 20, paddingTop: 60, marginHorizontal: 20, ...SHADOW.card },
