...
export default function PaymentDetail() {
  ...
  const chartData = datesArray.map((date, index) => {
    ...
    const itemsForDate = devicePayments.filter((item: any) => {
      if (!item?.trx_date) return false;
      const itemDate = new Date(item.trx_date).toLocaleDateString("en-CA");
      return itemDate === dateString;
    });

    const totalValue = itemsForDate.reduce((sum: number, item: any) => sum + Number(item.gross_amount), 0);
      ...
      <View style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <Animated.View entering={FadeInUp.delay(150)} style={styles.historySection}>
            ...
            {devicePayments.length === 0 ? <EmptyState title="Belum Ada Aktivitas" subtitle="Tidak ada riwayat penarikan" /> : 
              devicePayments.slice(0, 30).map((item: any, index: number) => (
                <View key={item.id || index} style={styles.historyItem}>
                  ...
                  <Text style={[styles.historyAmount, { color: COLORS.warning }]}>- Rp {Number(item.gross_amount).toLocaleString("id-ID")}</Text>
                </View>
              ))
            }
          </Animated.View>
