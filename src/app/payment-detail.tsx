...
export default function PaymentDetail() {
  ...
  const chartData = datesArray.map((date, index) => {
    ...
    const totalValue = itemsForDate.reduce(
      (sum: number, item: any) => sum + Number(item.gross_amount),
      0,
    );
          ...
          >
            ...
            {devicePayments.length === 0 ? (
              <EmptyState
                ...
                subtitle="Tidak ada riwayat penarikan"
              />
            ) : (
              devicePayments.slice(0, 30).map((item: any, index: number) => (
                <View key={item.id || index} style={styles.historyItem}>
                  ...
                  <Text
                    style={[styles.historyAmount, { color: COLORS.warning }]}
                  >
