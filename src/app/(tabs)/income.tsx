...
export default function IncomeScreen() {
  ...
  const historyQuery = useInfiniteQuery({
    queryKey: ["tenant", user?.id, "income", "history", rangeKey],
    queryFn: ({ pageParam }) =>
      fetchIncomeHistory({
        ...dateRange,
        page: pageParam,
  ...
  const summaryQuery = useQuery({
    queryKey: ["tenant", user?.id, "income", "summary", rangeKey],
    queryFn: () => fetchIncomeHistorySummary(dateRange),
    enabled: Boolean(user),
  });
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <EmptyState
                ...
                subtitle="Belum ada riwayat pendapatan di periode ini."
              />
            }
            ListFooterComponent={
              historyQuery.isFetchingNextPage ? (
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  accessibilityLabel={`Buka detail pendapatan ${device?.device_name ?? "perangkat"}`}
