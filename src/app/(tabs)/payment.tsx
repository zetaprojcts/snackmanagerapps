...
export default function PaymentScreen() {
  ...
  const totalPayment = useMemo(() => {
    return filteredPayments.reduce((sum: number, item: any) => sum + Number(item.gross_amount || 0), 0);
  }, [filteredPayments]);
      ...
      <View style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.totalCard}>
            ...
            <Text style={styles.totalValue}>Rp {totalPayment.toLocaleString("id-ID")}</Text>
          </View>
          ...
          <View style={styles.listContainer}>
            {filteredPayments.length === 0 && !isLoading ? (
              <EmptyState title="Tidak Ada Data" subtitle="Belum ada riwayat penarikan di periode ini." />
            ) : (
              filteredPayments.map((item: any, index: number) => {
                ...
                return (
                  <Animated.View key={item.id || index} entering={FadeInUp.delay(index * 50)} style={styles.listItem}>
                    ...
                    <Text style={styles.itemAmount}>- Rp {Number(item.gross_amount).toLocaleString("id-ID")}</Text>
                  </Animated.View>
                );
              })
            )}
...
const styles = StyleSheet.create({
  ...
  itemAmount: { fontSize: 15, fontWeight: "700", color: COLORS.warning }, // Menggunakan warning untuk penarikan
